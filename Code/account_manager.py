...
class League:
    ...
    def get_league_results(self):  # get league results
        ...
        response = json.loads(r.text)
        league_mode = League.get_league_description(self)['mode']
        results = []

        if League.get_league_mode(self) == "1on1":
            for match_info in response:
                ...
                if match.get_match_status() == "closed":
                    ...
                    result = "Match is still open - match id: {0}\n{1}; team ID: {2}\n{3}; team ID: {4}".format(match_id, player_a_name, player_a_id, player_b_name,player_b_id)
        else:
            for match_info in response:
                match_id = match_info['id']
                match = Match(match_id)
    ...
    def get_league_members_info(self):
        ...
        if League.get_league_mode(self) == "1on1":
            for contestant in response:
                league_members_info.append(contestant)
        else:
            for contestant in response:
        ...
        if League.get_league_mode(self) == "1on1":
            for match in response:
                ...
                if match_status == "protest":
                    ...
                    protest_dict = {"ticket number": tickets_amount,"match_id": match_id, player_a_id: player_a_name, player_b_id: player_b_name, "Check your tickets": admin_protests_link, "Check match": match_protest_link}
    ...
    def get_contestants_members_info(self):#players' useful info
        ...
        if is_error(response):
            ...
            if Match.get_match_mode(self) == "1on1":
                ...
                for i in range(len(contestants)):
                    contestant = User(contestants[i]['user']['id'])
                    members_info.append(contestant.get_useful_info())
            else:
                players_id = []
                i = 0
                for key, value in test_match.get_match_info().items():
        ...
        if is_error(response):
            ...
            if Match.get_match_mode(self) == "1on1":
                ...
                if user_lose.get_profile() == "error":
                    ...
                    user_lose_name = "'" + user_lose.get_profile()['nickname'] + "'"
                if result != "draw":
                    return "{0} has won! {1}\n{2} has lost. {3}".format(user_win_name, user_win_id, user_lose_name, user_lose_id)
                else:
                    return "The result is draw\n{0}: {1}\n{2}: {3}".format(user_win_name, user_win_id, user_lose_name, user_lose_id)
            else:
    ...
    if choice == "s":
        ...
        # MATCH FUNCTIONS
            ...
            if match_results == "open":
                ...
                print("There is no match media in this match")
            else:
