...
class Ui_signUp_window(object):
    ...
    def get_user_password(self, user_password):
        ...
        if user_password != "":
            if app_account_manager.main_page.data_modules.verifier.Verifier(user_password).check_password():
    ...
    def get_user_nickname(self):
        ...
        if user_nickname != "":
            if app_account_manager.main_page.data_modules.verifier.Verifier(user_nickname).check_password():
    ...
    def check_user_info(self):
        ...
        if user_email is not None and user_nickname is not None and user_password_1 is not None and user_password_2 is not None \
        and user_password_1 == user_password_2:
            return True
            ...
            if self.signUp_email_input.text() == "" or self.signUp_nickname_input.text() == "" \
                    or self.signUp_password_input.text() == "" or self.signUp_password_2_input.text() == "":
                QtWidgets.QMessageBox.warning(None, "Enter your info",
                                              "Fill in the all fields.")
            return False

    def signup_btn_pressed(self):
        login_page = app_account_manager.login_page.loginPage.loginPage
        login_page_ui = app_account_manager.login_page.loginPage.ui
        settings_ui = app_account_manager.settings_page.settings_page.ui
        ...
        if self.check_user_info():
            ...
            setup_info.write("User email: {0}\nUser nickname: {1}\nUser password: {2}\n"
                             "Send email state: {3}\nIs registered: {4}\n'Remember me' status: 0".format(email, nickname, password,
                                                                                                         send_email_state, 1))
            setup_info.close()
