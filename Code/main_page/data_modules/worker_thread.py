...
import time
import app_account_manager.main_page.data_modules.refresh_thread as _refresh_thread
import app_account_manager.main_page.data_modules.verifier as _verifier
import app_account_manager.main_page.data_modules.user as _user
import app_account_manager.main_page.data_modules.functions as _functions
import app_account_manager.main_page.data_modules.team as _team
import app_account_manager.main_page.data_modules.match as _match
import app_account_manager.main_page.data_modules.league as _league
...
class BackendQThread(QThread):
    ...
    def get_player_acc(self):  # Player Account
        """Prints all user's gameaccounts."""
    ...
    def get_match_members_profiles(self):
        """Prints all match members' profiles"""
        match_id = self.mainwindow.input_match_id.text()
        if match_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter match ID' ")
        else:
    ...
    def get_match_date(self):
        """Prints match date and its duration."""
        match_id = self.mainwindow.input_match_id.text()
        if match_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter match ID' ")
        else:
    ...
    def get_match_result(self):
        """Prints match status and result, if it is closed."""
        match_id = self.mainwindow.input_match_id.text()
        if match_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter match ID' ")
        else:
    ...
    def get_league_info(self):
        """Prints all league data (maybe even too much data)."""
    ...
    def get_league_results(self):
        """Prints league results - all matches' results."""
        league_id = self.mainwindow.input_league_id.text()
        if league_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter league ID' ")
        else:
    ...
    def get_league_contestants(self):
        """Prints info about all teams, participating in the tournament."""
        league_id = self.mainwindow.input_league_id.text()
        if league_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter league ID' ")
        else:
    ...
    def get_league_contestants_info(self):
        """Prints info about all users, participating in the tournament."""
        league_id = self.mainwindow.input_league_id.text()
        if league_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter league ID' ")
        else:
    ...
    def get_league_matches_gameaccounts(self):
        """Prints all matches and gameaccounts there."""
        league_id = self.mainwindow.input_league_id.text()
        if league_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter league ID' ")
        else:
    ...
    def auto_admin(self):
        ...
        setup_info = app_account_manager.email_handler.get_file_info()
        email_handler = app_account_manager.email_handler.EmailHandler(setup_info)
                        ...
                        if _league.League(league_id).get_league_description() == "error":
                            ...
                            while True:
                                ...
                                if text == "Total amount of opened tickets: {0}".format(total_protests_amount) + "\n":
                                    ...
                                    # ====_EMAIL_====
                                    if sounds_amount > 0:
                                        if app_account_manager.email_handler.get_send_email_status(app_account_manager.email_handler.get_file_info()):
                                            try:
    ...
    def get_suspects(self):
        """Prints all users, who has barrage or doesn't have a specific (e.g. 'Uplay') account."""
        league_id = self.mainwindow.input_league_id.text()
        if league_id == "":
            self.mainwindow.output_info.append("\nERROR: 'Enter league ID' ")
        else:
