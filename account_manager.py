    ...
    def get_contestants_members_info(self):#players' useful info
        ...
        for key, value in test_match.get_match_info().items():
            if key == "gameaccounts":
                ...
                #players_id = list(players.keys())
                #player_id = players_id[i]
                #new_member = User(player_id)
                #members_info.append(new_member.get_useful_info())
        return members_info
...
while True:
    ...
    if choice == "s":
        ...
        #MATCH FUNCTIONS
            ...
            print("TEST: ", new_match.get_match_media())
        #input()
